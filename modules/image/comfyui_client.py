...
def _build_sdxl_workflow(prompt: str, seed: int | None = None) -> dict:
    """SDXL base 工作流（使用 sd_xl_base_1.0.safetensors）。"""
    cfg = settings()["comfyui"]
    seed = seed or int(time.time() * 1000) % (2 ** 31)
    return {
        "4": {"class_type": "CheckpointLoaderSimple", "inputs": {
            "ckpt_name": cfg.get("default_model", "sd_xl_base_1.0.safetensors"),
        }},
...
def generate_images(script_path: Path, *, slug: str | None = None) -> list[Path]:
    """讀取 script.json，為每個段落生成圖片，回傳圖片路徑清單。

    優先順序：ComfyUI（本地 GPU）→ Gemini Imagen（API）→ 文字卡片佔位圖
    slug 有值時把進度寫入 EpisodeStatus.progress_detail。
    """
    # 優先嘗試 Gemini Imagen
    from modules.image.gemini_image_client import is_available as gemini_available
    from modules.image.gemini_image_client import generate_images as gemini_generate

    use_comfyui = _is_comfyui_running()
    use_gemini = not use_comfyui and gemini_available()

    if use_comfyui:
        _progress(slug, "🖼️ ComfyUI 已啟動，使用 GPU 生成")
    elif use_gemini:
        _progress(slug, "🖼️ 改用 Gemini Imagen API")
        return gemini_generate(script_path)
    else:
        _progress(slug, "🖼️ 無 ComfyUI / Gemini，使用文字卡片佔位圖")
    ...
    for i, sec in enumerate(sections, 1):
        ...
        if out.exists():
            _progress(slug, f"🖼️ 圖片 {i}/{total} 已快取，跳過")
            paths.append(out)
            continue

        if use_comfyui:
            try:
                _progress(slug, f"🖼️ 圖片 {i}/{total}（送 ComfyUI 排隊）")
                workflow = _build_sdxl_workflow(prompt)
                pid = _submit_prompt(workflow)
                _progress(slug, f"🖼️ 圖片 {i}/{total}（ComfyUI 算圖中，pid={pid[:8]}）")
                img_bytes = _wait_for_image(pid)
                if img_bytes:
                    out.write_bytes(img_bytes)
                    _progress(slug, f"🖼️ 圖片 {i}/{total} ✅")
                else:
                    logger.warning(f"段落 {sid} 生成超時，改用文字卡片")
                    _make_placeholder(sid, sec.get("type", ""), out)
            except Exception as e:
                logger.error(f"段落 {sid} ComfyUI 失敗：{e}，改用文字卡片")
                _make_placeholder(sid, sec.get("type", ""), out)
        else:
            _progress(slug, f"🖼️ 圖片 {i}/{total}（文字卡片）")
            _make_placeholder(sid, sec.get("type", ""), out)

        paths.append(out)

    _progress(slug, f"✅ 圖片生成完成（{len(paths)} 張）")
    return paths
